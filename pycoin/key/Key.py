...
class Key(object):
    def __init__(self, secret_exponent=None, public_pair=None, hash160=None,
        ...
        if self._public_pair is None and self._secret_exponent is not None:
            if self._secret_exponent < 1 \
                    or self._secret_exponent >= generator_secp256k1.order():
                raise InvalidSecretExponentError()
            public_pair = self._secret_exponent * generator_secp256k1
            self._public_pair = public_pair

        if self._public_pair is not None \
                and (None in self._public_pair or
                     not generator_secp256k1.contains_point(*self._public_pair)):
    ...
    def sign(self, h):
        ...
        val = from_bytes_32(h)
        r, s = generator_secp256k1.sign(self.secret_exponent(), val)
    ...
    def verify(self, h, sig):
        ...
        if self.public_pair() is None:
            ...
            # our key
            possible_pubkeys = generator_secp256k1.possible_public_pairs_for_signature(val, rs)
            ...
            for candidate in possible_pubkeys:
                ...
                return False
        return generator_secp256k1.verify(pubkey, val, rs)
