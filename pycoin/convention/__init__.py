...
def satoshi_to_btc(satoshi_count: int) -> decimal.Decimal:
    if satoshi_count == 0:
...
def btc_to_satoshi(btc: Any) -> int:
    return int(decimal.Decimal(btc) * SATOSHI_PER_COIN)


def satoshi_to_mbtc(satoshi_count: int) -> decimal.Decimal:
    if satoshi_count == 0:
        return decimal.Decimal(0)
...
def mbtc_to_satoshi(btc: Any) -> int:
