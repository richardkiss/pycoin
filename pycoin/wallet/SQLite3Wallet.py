...
class SQLite3Wallet(object):
    ...
    def create_payables(
        ...
        if self._desired_spendable_count is not None:
            if (
                self.persistence.unspent_spendable_count()
                ...
                if (
                    change_amount
                    > desired_change_output_count * self._min_extra_spendable_amount
                ):
    ...
    def create_unsigned_send_tx(self, address, amount):
        ...
        with self._lock:
            ...
            while (
                confirmations > 0
                and self.get_balance(confirmations=confirmations)
    ...
    def get_balance(self, confirmations=1):
        ...
        with self._lock:
            balance = 0
            for s in self.persistence.unspent_spendables(
                lbi, confirmations=confirmations
            ):
