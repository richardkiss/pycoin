...
def load_library():
    try:
        ...
        if PYCOIN_LIBSECP256K1_PATH:
            warnings.warn(
                "PYCOIN_LIBSECP256K1_PATH set but libsecp256k1 optimizations not loaded"
            )
        return None
...
class Optimizations:
    ...
    def sign(self, secret_exponent, val, gen_k=None):
        ...
        if gen_k is not None:
            ...
            nonce_function = CFUNCTYPE(
                c_int, p_b32, p_b32, p_b32, POINTER(c_byte * 16), c_void_p, c_uint
            )(adaptor)
    ...
    def verify(self, public_pair, val, signature_pair):
        ...
        public_pair_bytes = (
            b"\4" + to_bytes_32(public_pair[0]) + to_bytes_32(public_pair[1])
        )
    ...
    def multiply(self, p, e):
        ...
        if not r:
            return False
        r = libsecp256k1.secp256k1_ec_pubkey_tweak_mul(
