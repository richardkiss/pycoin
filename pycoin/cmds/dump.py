...
def dump_inputs(output, tx, network, verbose_signature, traceback_f, disassembly_level):
    for idx, tx_in in enumerate(tx.txs_in):
        if tx.is_coinbase():
            output.append(
                "%4d: COINBASE  %12.5f m%s"
                % (idx, satoshi_to_mbtc(tx.total_in()), network.symbol)
            )
            continue
        suffix = ""
    ...
    if signatures:
        ...
        for sig_pair, sig_type, sig_hash in signatures:
            ...
            if not sig_types_identical and tx_out:
                output.append(
                    "      z{}: {:#066x} {}".format(
                        i, sig_hash, network.annotate.sighash_type_to_string(sig_type)
                    )
                )
            if i:
                i += 1
...
def dump_footer(network, output, tx, missing_unspents):
    ...
    if 1:
        output.append(
            "Total output %12.5f m%s"
            % (satoshi_to_mbtc(tx.total_out()), network.symbol)
        )
    if not missing_unspents:
        output.append(
            "Total fees   %12.5f m%s" % (satoshi_to_mbtc(tx.fee()), network.symbol)
        )
...
def dump_tx(
    ...
    for idx, tx_out in enumerate(tx.tx_outs_as_spendable()):
        ...
        output.append(
            "%4d: %34s receives %12.5f m%s"
            % (idx, address, amount_mbtc, network.symbol)
        )
        if disassembly_level > 0:
            for (
                pre_annotations,
