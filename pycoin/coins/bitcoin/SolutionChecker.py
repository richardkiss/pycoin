...
class BitcoinSolutionChecker(SegwitChecker, P2SChecker):
    ...
    def _solution_script_to_stack(self, tx_context, flags, traceback_f):
        ...
        vm = self.VM(
            tx_context.solution_script,
            tx_context,
            self._make_sighash_f(tx_context.tx_in_idx),
            f1,
        )
    ...
    def _tx_in_for_idx(self, idx, tx_in, tx_out_script, unsigned_txs_out_idx):
        ...
        return self.tx.TxIn(
            tx_in.previous_hash, tx_in.previous_index, b"", tx_in.sequence
        )
    ...
    def _signature_hash(self, tx_out_script, unsigned_txs_out_idx, hash_type):
        ...
        tx_out_script = self.delete_subscript(
            tx_out_script, self.ScriptTools.compile("OP_CODESEPARATOR")
        )
    ...
    def puzzle_and_solution_iterator(self, tx_context, flags=None, traceback_f=None):
        ...
        solution_stack = self._solution_script_to_stack(
            tx_context, flags=flags, traceback_f=traceback_f
        )
