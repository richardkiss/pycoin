    ...
    def _check_witness_program_v0(self, witness_solution_stack, witness_program):
        ...
        if size == 32:
            if len(witness_solution_stack) == 0:
                raise ScriptError(
                    "witness program witness empty", errno.WITNESS_PROGRAM_WITNESS_EMPTY
                )
            puzzle_script = witness_solution_stack[-1]
            if sha256(puzzle_script).digest() != witness_program:
                raise ScriptError(
                    "witness program mismatch", errno.WITNESS_PROGRAM_MISMATCH
                )
            ...
            raise ScriptError(
                "witness program wrong length", errno.WITNESS_PROGRAM_WRONG_LENGTH
            )
    ...
    def witness_program_tuple(
        ...
        if witness_version is None:
            ...
            if len(solution_stack) > 0:
                err = (
                    errno.WITNESS_MALLEATED_P2SH if is_p2sh else errno.WITNESS_MALLEATED
                )
