...
class ValidationTest(unittest.TestCase):
    ...
    def test_validate_block_data(self):
        ...
        disassembly = network.script_tools.disassemble(tx_out.script)
        tx_out.script = network.script_tools.compile(disassembly)
    ...
    def test_validate_two_inputs(self):
        ...
        disassembly = network.script_tools.disassemble(tx_to_validate.txs_in[0].script)
        tx_to_validate.txs_in[0].script = network.script_tools.compile(disassembly)
        self.assertEqual(tx_to_validate.bad_solution_count(), 0)
        ...
        tx_to_validate.unspents_from_db(TX_DB)
        self.assertEqual(tx_to_validate.bad_solution_count(), 0)
        disassembly = network.script_tools.disassemble(tx_to_validate.txs_in[1].script)
        disassembly = disassembly.replace("960c258ffb494d2859f", "960d258ffb494d2859f")
        tx_to_validate.txs_in[1].script = network.script_tools.compile(disassembly)
        self.assertEqual(tx_to_validate.bad_solution_count(), 1)
        ...
        disassembly = network.script_tools.disassemble(tx_1.txs_out[0].script)
        disassembly = disassembly.replace("4cf9d8545548de0256f48c4b0726b14294619267",
                                          "4cf9d8545548de1256f48c4b0726b14294619267")
        tx_1.txs_out[0].script = network.script_tools.compile(disassembly)
        ...
        disassembly = network.script_tools.disassemble(tx_2.txs_out[0].script)
        disassembly = disassembly.replace("a645d6ccbefe5f11b8050f5624b5a054f734135b",
                                          "a665d6ccbefe5f11b8050f5624b5a054f734135b")
        tx_2.txs_out[0].script = network.script_tools.compile(disassembly)
