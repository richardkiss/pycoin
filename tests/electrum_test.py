...
class ElectrumTest(unittest.TestCase):
    def test_initial_key(self):
        ...
        for idx, address in enumerate(RECEIVING_ADDRESSES):
            ...
            calculated_address = subkey.address(netcode=nc)
            self.assertEqual(address, calculated_address)
            wif = subkey.wif(netcode=nc)
            key, netcode = key_from_text(secp256k1_generator, wif)
            self.assertEqual(key.address(use_uncompressed=True, netcode=nc), address)
