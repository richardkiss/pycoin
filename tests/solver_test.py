...
from pycoin.ui.ui import address_for_pay_to_script, script_for_p2s, script_for_multisig, script_for_p2pk, script_for_p2pkh
...
class SolverTest(unittest.TestCase):
    ...
    def make_test_tx(self, input_script):
        ...
        txs_out = [TxOut(1000, script_for_p2pkh(Key(1, generator=secp256k1_generator).hash160()))]
    ...
    def test_p2pkh(self):
        ...
        self.do_test_tx(script_for_p2pkh(key.hash160()))
    ...
    def test_nonstandard_p2pkh(self):
        key = Key(1, generator=secp256k1_generator)
        self.do_test_tx(BitcoinScriptTools.compile("OP_SWAP") + script_for_p2pkh(key.hash160()))
    ...
    def test_p2sh(self):
        ...
        script = script_for_p2s(underlying_script)
        self.do_test_tx(script, p2sh_lookup=build_p2sh_lookup([underlying_script]))

        underlying_script = BitcoinScriptTools.compile("OP_SWAP") + script_for_p2pkh(keys[0].hash160())
        script = script_for_p2s(underlying_script)
        self.do_test_tx(script, p2sh_lookup=build_p2sh_lookup([underlying_script]))
    ...
    def test_p2multisig_wit(self):
        ...
        script = script_for_p2s(p2sh_script)
