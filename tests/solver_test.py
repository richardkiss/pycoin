...
class SolverTest(unittest.TestCase):
    ...
    def make_test_tx(self, input_script):
        ...
        txs_out = [
            TxOut(1000, network.contract.for_address(network.keys.private(1).address()))
        ]
    ...
    def do_test_tx(self, incoming_script, **kwargs):
        ...
        tx_in_idx = 0
        kwargs["hash160_lookup"] = network.tx.solve.build_hash160_lookup(
            (k.secret_exponent() for k in keys)
        )
        kwargs["generator_for_signature_type_f"] = (
            Solver.SolutionChecker.VM.generator_for_signature_type
        )
    ...
    def test_nonstandard_p2pkh(self):
        key = network.keys.private(1)
        self.do_test_tx(
            network.script.compile("OP_SWAP")
            + network.contract.for_address(key.address())
        )
    ...
    def test_p2sh(self):
        ...
        script = network.contract.for_address(
            network.address.for_p2s(underlying_script)
        )
        self.do_test_tx(
            script, p2sh_lookup=network.tx.solve.build_p2sh_lookup([underlying_script])
        )

        underlying_script = network.script.compile(
            "OP_SWAP"
        ) + network.contract.for_address(keys[0].address())
        script = network.contract.for_address(
            network.address.for_p2s(underlying_script)
        )
        self.do_test_tx(
    ...
    def test_p2multisig_wit(self):
        ...
        secs = [k.sec() for k in keys]
        underlying_script = network.contract.for_multisig(2, secs)
        p2sh_script = network.script.compile(
            "OP_0 [%s]" % b2h(hashlib.sha256(underlying_script).digest())
        )
    ...
    def test_p2multisig_incremental(self):
        ...
        for k in keys:
            try:
                ...
                pass
            kwargs = {
                "hash160_lookup": network.tx.solve.build_hash160_lookup(
                    [k.secret_exponent()]
                )
            }
            kwargs["existing_script"] = [
                data
                for opcode, data, pc, new_pc in network.script.get_opcodes(
                    tx.txs_in[tx_in_idx].script
                )
                if data is not None
            ]
            kwargs["nocheck"] = True
