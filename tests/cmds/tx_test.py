...
class TxTest(ToolTest):
    ...
    def test_tx_fetch_unspent(self):
        ...
        FETCH_UNSPENT_EXPECT = [
            "5fd3d8275afb5b5cc202ae8480daefa4fe16d0cf480ce78545d6dc06c6fb101a/1/%s/1/0/0/0"
            % P2,
            "b9a84cffd3766bb642a697065b477eed032e36c377db80faac79b18e61b43b0d/1/%s/1/0/0/0"
            % P2,
            "0986d70aaa03213135998cf1a9b8a33012c033c6607584e84b8ae33d49fadce3/0/%s/1/0/0/0"
            % P2,
            "d658ab87cc053b8dbcfd4aa2717fd23cc3edfe90ec75351fadd6a0f7993b461d/7/%s/911/0/0/0"
            % P2,
        ]
    ...
    def test_cache_tx(self):
        ...
        self.launch_tool(
            "tx -C %s --db %s" % (tx.id(), tx.as_hex()),
            env=dict(PYCOIN_CACHE_DIR=the_dir),
        )
        self.assertTrue(
            os.path.exists(os.path.join(the_dir, "txs", "%s_tx.bin" % tx.id()))
        )
