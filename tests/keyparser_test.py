...
class KeyParserTest(unittest.TestCase):
    def test_parse_bip32_prv(self):
        ...
        self.assertEqual(
            key.address(is_compressed=False), "1MwkRkogzBRMehBntgcq2aJhXCXStJTXHT"
        )
    ...
    def test_parse_p2wpkh_pub_xtn(self):
        key = XTN.parse(
            "vpub5UFa1gPZYUUvrtiRnW6ZLPXLheCKL7NNhmemq5hJYsvBbANm8YorXbS2goXnySq2CNzNTi6FH2medK918d3hgmDgJN3F2cxNPoeyynGsdaQ"
        )
    ...
    def test_parse_p2wpkh_in_p2sh_pub_xtn(self):
        key = XTN.parse(
            "upub5E6ugGxVutPUyK12NdvrA36uNUPiXqjtEiPDeixpKxFn53kYUwzJf56UWKFM7dZWkcDret3cMMFjNMC8WXs5VWc5Wvgc631KGDNxDyuVkEH"
        )
        self.assertEqual(key.secret_exponent(), None)
        subkey = key.subkey_for_path("0/0")
    ...
    def test_parse_electrum_seed(self):
        ...
        subkey = key.subkey("1")
        self.assertEqual(
            subkey.wif(), "5KYqyRxoMGnwsXfEFWtVifAKTzU9RcAZu1hme6GLMECKdWHybns"
        )
