...
class EncodingTestCase(unittest.TestCase):
    def test_to_from_long(self):
        ...
        do_test(
            1460765565493402645157733592332121663123460211377,
            1,
            h2b("00ffdefe4f4875cf119fc3d8f4a09ae37ec4cc42b1"),
            256,
        )
    ...
    def test_to_from_base58(self):
        ...
        do_test(
            "1CASrvcpMMTa4dz4DmYtAqcegCtdkhjvdn",
            h2b("007a72b6fa63de36c4abc60a68b52d7f33e3d7cd3ec4babd39"),
        )
        do_test(
            "1111111111111111aaaa11aa",
    ...
    def test_to_from_hashed_base58(self):
        ...
        do_test(
            "11111111111111114njGbaozZJui9o",
            h2b("00000000000000000000000000000000436e7a51290b"),
        )
        do_test(
            "1mLRia5CbfDB9752zxvtrpnkigecaYWUSQNLJGECA8641ywusqomjhfdb6EM7bXGj1Gb",
            h2b(
                "000111d38e5fc9071ffcd20b4a763cc9ae4f252bb4e48fd66a835e252ada93ff480d6dd43dc62a641155a561616161"
            ),
        )
    ...
    def test_public_pair_to_sec(self):
        def do_test(
            as_public_pair, as_sec, is_compressed, as_hash160_sec, as_bitcoin_address
        ):
            self.assertEqual(
                sec_to_public_pair(as_sec, secp256k1_generator), as_public_pair
            )
            self.assertEqual(
                public_pair_to_sec(as_public_pair, compressed=is_compressed), as_sec
            )
            self.assertEqual(is_sec_compressed(as_sec), is_compressed)
            self.assertEqual(
                public_pair_to_hash160_sec(as_public_pair, compressed=is_compressed),
                as_hash160_sec,
            )

            self.assertEqual(
                network.address.for_p2pkh(as_hash160_sec), as_bitcoin_address
            )
            self.assertIsNotNone(network.parse.address(as_bitcoin_address))
            bad_address = (
                as_bitcoin_address[:17]
                + chr(ord(as_bitcoin_address[17]) + 1)
                + as_bitcoin_address[18:]
            )
        ...
        for public_pair, sec, compressed, hash160_sec, bitcoin_address in SEC_TEST_DATA:
            do_test(
                public_pair, h2b(sec), compressed, h2b(hash160_sec), bitcoin_address
            )
