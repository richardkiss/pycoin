...
class SignTest(unittest.TestCase):
    ...
    def multisig_M_of_N(self, M, N, unsigned_id, signed_id):
        ...
        script = network.contract.for_multisig(
            m=M, sec_keys=[key.sec() for key in keys[:N]]
        )
        tx_out = TxOut(1000000, script)
        tx1 = Tx(version=1, txs_in=[tx_in], txs_out=[tx_out])
        tx2 = network.tx_utils.create_tx(
            tx1.tx_outs_as_spendable(), [keys[-1].address()]
        )
        ...
        hash160_lookup = network.tx.solve.build_hash160_lookup(
            (key.secret_exponent() for key in keys[:M])
        )
    ...
    def test_p2sh_multisig_sequential_signing(self):
        ...
        for ordered_keys in [(key_1, key_2), (key_2, key_1)]:
            ...
            txs_out = [
                TxOut(
                    10000,
                    network.contract.for_address("3KeGeLFmsbmbVdeMLrWp7WYKcA3tdsB4AR"),
                )
            ]
            ...
            for key in ordered_keys:
                ...
                tx.sign(
                    network.tx.solve.build_hash160_lookup([key.secret_exponent()]),
    ...
    def test_sign_pay_to_script_multisig(self):
        ...
        keys = [network.keys.private(secret_exponent=i) for i in range(1, N + 2)]
        tx_in = TxIn.coinbase_tx_in(script=b"")
        underlying_script = network.contract.for_multisig(
            m=M, sec_keys=[key.sec() for key in keys[:N]]
        )
        ...
        hash160_lookup = network.tx.solve.build_hash160_lookup(
            (key.secret_exponent() for key in keys[:N])
        )
